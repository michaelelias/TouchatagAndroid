...
public class ApplicationStore {
	...
	public Page<Application> getPage(int pageNumber, int pageSize) {
		Page<Application> page = new Page<Application>();
		SQLiteDatabase db = null;
		try {
			db = dbOpenHelper.getReadableDatabase();
			String limitClause = ((pageNumber - 1) * pageSize) + ", " + pageSize;
			Cursor cursor = db.query(TABLE_NAME, new String[] { Columns.ID, Columns.OWNERID, Columns.CREATED, Columns.SPECIFICATION, Columns.NAME, Columns.DESCRIPTION, Columns.TEMPLATE }, null, null, null, null, null, limitClause);
	...
	public Application findByIdentifier(String appIdentifier) {
		...
		try {
			db = dbOpenHelper.getReadableDatabase();
			Cursor cursor = db.query(TABLE_NAME, new String[] { Columns.ID, Columns.OWNERID, Columns.CREATED, Columns.SPECIFICATION, Columns.NAME, Columns.DESCRIPTION, Columns.TEMPLATE }, Columns.ID + "=?", new String[] { appIdentifier }, null, null,
					null);
			if (cursor.moveToFirst()) {
	...
	public List<Application> findByIdentifierNotIn(List<String> identifiers) {
		...
		try {
			...
			Cursor cursor = db.query(TABLE_NAME, new String[] { Columns.ID, Columns.OWNERID, Columns.CREATED, Columns.SPECIFICATION, Columns.NAME, Columns.DESCRIPTION, Columns.TEMPLATE }, Columns.ID + " NOT IN (" + inClause + ")", null, null, null, null);
			if (cursor.moveToFirst()) {
				while (!cursor.isAfterLast()) {
					Application app = createAppFromCursor(cursor);
					apps.add(app);
	...
	private Application createAppFromCursor(Cursor cursor) throws Exception{
	...
	public List<Application> findAll() {
		...
		try {
			db = dbOpenHelper.getReadableDatabase();
			Cursor cursor = db.query(TABLE_NAME, new String[] { Columns.ID, Columns.OWNERID, Columns.CREATED, Columns.SPECIFICATION, Columns.NAME, Columns.DESCRIPTION, Columns.TEMPLATE }, null, null, null, null, null);
			if (cursor.moveToFirst()) {
				while (!cursor.isAfterLast()) {
					Application app = createAppFromCursor(cursor);
